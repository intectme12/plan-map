...
**진행 중 (Phase 2) / 다음 세션 할 일**
- 카카오맵 실 연동: 사용자가 새 키 발급 후 `.env`에 `NEXT_PUBLIC_KAKAO_JS_KEY`/`KAKAO_REST_API_KEY` 입력 필요(현재는 위경도 수동 입력)
- 대중교통(ODsay/Tmap) 연동은 별도 API 키 발급이 필요해 자동 진행 범위에서는 코드 인터페이스만 마련하고 실제 호출은 키 설정 후로 보류
