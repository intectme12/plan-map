...
- **해결**: `window.kakao.maps.event.preventMap(card)` — 카카오맵 API가 커스텀 오버레이의 인터랙티브 콘텐츠(버튼 등)를 위해 공식 제공하는 함수로, 카드 안에서 일어나는 클릭이 지도까지 전파되지 않게 막는다. 인증 우회 같은 게 아니라 카카오맵 SDK가 정확히 이 문제를 위해 만들어둔 API
