...
- **후속 요청(같은 세션)**: "여행계획 만드는 지도 화면"(=`/trips` 페이지)에서 메시지 확인이 더 눈에 띄게 해달라는 요청 — 헤더 우측 구석의 작은 "메시지" 링크였던 걸 `TripsTabs.tsx`의 탭 줄로 옮겨서 내 여행계획/다른 사람 여행계획 바로 옆(사용자가 요청한 순서: 내 여행계획 → 다른 사람 여행계획 → 메시지 → 나에게 공유됨 → 회원검색)에 안읽음 배지와 함께 배치. `MessageNavLink.tsx`에 `className` prop을 추가해 헤더 스타일/탭 스타일 양쪽에서 재사용(중복 컴포넌트 안 만듦). 브라우저로 탭 순서와 `/messages`로 정상 이동하는 것 확인, 테스트 계정 삭제해 정리
- **후속 요청 2건(같은 세션)**: MESSAGING.md의 "미구현/보류" 목록 중 회원검색 메시지 버튼 + 읽음/타이핑 표시를 요청, 진행 중 사용자가 카카오톡 아이콘 배지 같은 것도 추가 요청.
  - `UserSearchBrowser.tsx` 검색 결과 카드에 `SendMessageButton` 직접 추가(프로필 화면까지 안 들어가도 됨) — 카드 전체를 감싸던 `<Link>`를 아바타/닉네임 영역만 감싸도록 좁히고 버튼을 형제로 뺌(SharedTripCard.tsx와 같은 패턴)
  - **타이핑 중 표시**: `lib/messageEvents.ts`의 pub/sub을 이벤트 이름으로 구분되는 범용 `broadcast()` 위에 `publishTyping`/`publishRead`를 얹어 확장. 입력창에서 2초 스로틀로 `POST .../typing` 신호를 보내고, 받는 쪽은 4초 무신호 시 자동으로 지움(별도 "그만 입력" 신호 없이 타임아웃만으로 처리)
  - **읽음 표시**: 이미 있던 `userALastReadAt`/`userBLastReadAt`를 그대로 활용 — 상대가 `POST .../read` 호출 시 `read` SSE 이벤트로 실시간 통지, 내가 보낸 메시지 중 가장 최근 것에만 "읽음" 표시(카카오톡처럼, 전부 표시하면 지저분함)
...
**완료 (2026-09-09, 메시지 탭을 헤더 버튼으로 이동)**

사용자 요청으로 "내 여행계획" 페이지의 메시지 진입점을 `TripsTabs.tsx` 탭 줄에서 다시 페이지 헤더로 이동 — "내 정보"/"로그아웃" 버튼 왼쪽에 같은 스타일(테두리 버튼)로 배치. 지난 세션(2026-09-09, DM 후속 요청)에서 헤더 → 탭 줄로 옮겼던 것을 되돌리는 요청.

- [trips/page.tsx](apps/web/src/app/trips/page.tsx): 헤더의 `flex justify-end` 버튼 그룹에 `<MessageNavLink>`를 "내 정보" 앞에 추가(기본 `className`이 이미 이 자리에 맞게 만들어져 있던 걸 그대로 사용 — 컴포넌트 자체 주석에도 원래 이 자리 용도라고 적혀 있었음)
...
**완료 (2026-09-09, 실제 GROQ_API_KEY로 Groq 추출 품질 검증)**

사용자가 실제 `GROQ_API_KEY`를 `.env`에 넣고 강릉 2박 3일 여행 후기 텍스트(이모지/표 형식 포함)로 테스트 요청. 코드 변경은 없음 — 순수 검증 세션.

- 브라우저(자동화 패널)로 dev 서버의 `/api/trips/{id}/ai-parse`를 호출했더니 매번 `SELF_SIGNED_CERT_IN_CHAIN`으로 500 — 아래 "네이버 로그인 invalid_code"와 **동일한 원인·동일한 에러 코드**(이 환경의 보안 소프트웨어가 아웃바운드 HTTPS를 가로챔). 다만 **같은 코드·같은 키로 `npx tsx --env-file=.env`(dotenv 없이 Node 24 내장 `--env-file`로 ESM import 순서 문제 회피) 독립 스크립트를 돌리면 정상 동작**하는 것까지 확인 — 즉 이 프리뷰 브라우저에 연결된 dev 서버 프로세스에서 만드는 아웃바운드 연결만 막히고, 같은 머신의 일반 `node` 프로세스는 안 막힘. 원인은 여전히 이 PC/환경 특유의 것으로 보이고(프록시 환경변수는 전부 비어있음을 확인, IPv4/IPv6 강제도 차이 없음 — 정확한 차단 기준은 특정 못 함), 실제 배포 서버나 사용자 본인 브라우저에서는 재현 안 될 가능성이 높음
- 대신 실제 `aiParse.ts`/`aiImport.ts` 코드를 `tsx`로 직접 실행해 검증: 사용자가 준 강릉 텍스트(표 형식 + 이모지 섞인 일정표)에서 초당순두부·오죽헌·경포해변·경포호수·안목해변·엄지네포장마차 본점·주문진항·주문진 해변·영진해변·사천해변·정동진 해변·모래시계공원·강릉중앙시장·강릉월화거리·강릉역까지 **15개 장소를 정확히 추출**, 카테고리(음식점/관광지/해변/공원/시장/거리/교통)와 시간 메모까지 자연스럽게 채움 — 표 형식·이모지가 섞인 지저분한 원문에서도 잘 뽑아냄. 짧은 텍스트로 지오코딩까지 포함한 전체 파이프라인(`parseTripText`)도 별도로 돌려 오죽헌/경포해변/안목해변 커피거리/엄지네포장마차 본점 전부 실제 강릉 주소로 정확히 매칭되는 것까지 확인 — Groq 전환이 실제로 잘 동작함을 확인
- 테스트에 쓴 계정/트립은 삭제해 정리. 임시 검증 스크립트(`scratch_test_*.mts`)도 커밋 전에 삭제 — 저장소에 남은 변경 없음

**완료 (2026-09-09, SELF_SIGNED_CERT_IN_CHAIN 근본 원인 해결 — dev 서버에 `--use-system-ca` 적용)**

사용자가 실제 브라우저(이 자동화 프리뷰가 아니라 본인 환경)에서 "AI 분석하기"를 눌렀더니 그대로 "서버 오류가 발생했습니다"가 재현됨 — 바로 앞 세션에서 "이 자동화 브라우저 프리뷰에서만 막히고 사용자 본인 브라우저에서는 될 것"이라고 적었던 결론이 **틀렸음이 확인됨**(사용자도 같은 dev 서버 프로세스에 접속하는 것이었으므로 당연히 같은 문제를 겪음 — 정정).
...
**완료 (2026-09-11, 지도 마커 팝업 후기 버튼 위치 조정 + 클릭 시 팝업이 닫혀버리던 버그 수정)**

사용자가 실제로 써보고 알려준 문제 2건: (1) "후기 N개" 버튼을 별점 오른쪽으로 옮겨달라, (2) 버튼을 누르면 모달이 안 뜨고 그냥 정보창이 닫혀버림.

- **위치**: `titleRow`(이름+별점이 있는 줄) 밖에 별도 줄로 붙어있던 "후기 N개" 버튼을 `titleRow` 안으로 옮겨서 별점 배지 바로 오른쪽에 나란히 오도록 함(`display:block` → 인라인, `margin-bottom` 제거)
- **버그 원인**: 카카오맵 `CustomOverlay` 안의 DOM 클릭이 오버레이 내부에서만 처리되는 게 아니라 지도 자체의 click 이벤트로도 같이 전달된다(카카오맵 SDK 특성) — 그래서 "후기 N개"를 눌러도, 지도 전체에 걸어둔 "빈 곳 클릭하면 정보창 닫기" 리스너(`infoOverlayRef.current?.setMap(null)`)가 동시에 발동해서 버튼 자체의 동작이 묻혀버린 것. 원래 있던 ✕ 닫기 버튼이나 카카오맵/네이버지도 링크는 "발동해도 어차피 문제없는" 동작(닫기는 닫히는 게 정상 동작이고, `<a target="_blank">` 링크는 새 탭이 뜨는 것 자체는 막히지 않음)이라 이 문제가 안 드러났던 것
- **해결**: `window.kakao.maps.event.preventMap(card)` — 카카오맵 API가 커스텀 오버레이의 인터랙티브 콘텐츠(버튼 등)를 위해 공식 제공하는 함수로, 카드 안에서 일어나는 클릭이 지도까지 전파되지 않게 막는다. 인증 우회 같은 게 아니라 카카오맵 SDK가 정확히 이 문제를 위해 만들어둔 API (**틀렸음 — 2026-09-11에 실제 SDK 소스를 까보고 정정, 아래 해당 날짜 로그 참고**)
- **검증 방법 개선**: 이전 세션에서 마커 클릭 상호작용 검증이 자꾸 실패했던 이유를 이번에 알아냄 — 카카오맵 `CustomOverlay`의 콘텐츠 DOM은 `document.contains()`로 확인하면 `false`가 나오는 방식으로 렌더링됨(자체 오버레이 레이어), 그래서 `document.querySelectorAll(...)`로 오버레이 안의 버튼을 찾으려던 이전 시도들이 계속 실패했던 것. 이번엔 `CustomOverlay` 생성자를 가로채서 실제 콘텐츠 DOM 노드 참조를 직접 붙잡아두고, 그 노드 안에서 버튼을 찾아 클릭하는 방식으로 안정적으로 검증 성공
- 브라우저(테스트 계정 1개)로: 별점 옆에 "후기 1개" 버튼이 나란히 뜨는 것 확인 → 버튼 클릭 후 `overlay.getMap()`이 여전히 `null`이 아님(=정보창이 안 닫힘) 확인 → 동시에 모달에 "testplace 후기" 제목과 실제 후기 내용("Nice")이 뜨는 것까지 확인. `tsc --noEmit`/`eslint` 통과(기존 경고만 그대로). 테스트 계정 삭제해 정리
- 이 과정에서 `.next` 빌드 캐시가 깨져서(오늘 여러 번 `rm -rf .next` + 프로세스 강제종료를 반복한 영향으로 추정) `/login` 등 모든 페이지가 404로 응답하는 현상 발견 → `.next` 삭제 후 서버 재시작으로 해결. 재현되면 같은 방법으로 해결

**완료 (2026-09-11, `preventMap` 오사용 정정 — 실제로는 안 고쳐졌던 마커 정보창 버그 재수정)**

바로 위 항목에서 "고쳤다"고 기록했던 마커 팝업 후기 버튼 버그가, 사용자가 캐시까지 다 지우고 재시도해봐도 여전히 재현된다고 알려줌. 자동화 브라우저의 클릭 검증은 통과했었는데 실사용에서는 안 됐던 게 이상해서, 카카오맵 SDK 실제 소스를 직접 받아서(`https://dapi.kakao.com/v2/maps/sdk.js` → 로더가 동적으로 불러오는 실제 엔진 `t1.daumcdn.net/mapjsapi/js/main/4.5.26/kakao.js`) `preventMap`의 진짜 구현을 확인함.

- **진짜 원인**: `kakao.maps.event.preventMap`은 **인자를 받는 함수가 아니다** — `La: function(){ u.Qe = true; setTimeout(u.Na, 0) }`(내부 압축 코드)로, 호출된 순간부터 다음 tick까지만 지도의 클릭/드래그 처리를 잠깐 억제하는 **전역 플래그**다. 바로 위 세션에서 카드를 만들 때(`buildInfoCard` 끝) `preventMap(card)`를 **한 번만** 호출했는데, 이 플래그는 호출 직후 거의 바로 꺼져버려서 실제 사용자가 나중에 버튼을 클릭하는 시점엔 이미 아무 효과가 없었음 — 엉뚱한 인자(`card`)를 넘긴 것도 그냥 무시됨(JS는 초과 인자를 조용히 버림)
- **자동화 브라우저 검증이 왜 "통과"로 나왔는지도 같이 알아냄**: `element.click()`으로 쏘는 합성 클릭은 카카오맵의 실제 지도-클릭 판정 로직(마우스다운/업 좌표 추적 기반으로 추정)을 애초에 타지 않는 것으로 보임 — 즉 이 특정 버그는 이 자동화 브라우저로는 재현/검증이 원천적으로 안 되는 종류였음. 앞으로 카카오맵 클릭-전파 관련 버그는 이 방식으로 검증됐다고 과신하지 말 것
- **진짜 수정**: `preventMap()`을 카드 생성 시점이 아니라 **"후기 N개" 버튼의 `onclick` 핸들러 맨 앞에서, 인자 없이** 호출하도록 이동 — 클릭이 실제로 일어나는 그 순간에 플래그를 세워야, 그 이벤트가 버블링되어 지도의 "빈 곳 클릭 시 닫기" 리스너에 도달하는 시점(같은 동기 실행 안)에도 플래그가 여전히 켜져 있어 실제로 억제됨
- 검증은 자동화 브라우저로는 의미가 없다고 판단해 생략 — 사용자에게 실제 클릭으로 재확인 요청 예정

**완료 (2026-09-16, 로그인 후 홈 화면을 여행 발견/추천 플랫폼 스타일로 신규 구축)**

기존 `/`는 로그인 여부만 보고 `/trips`(관리자 페이지 느낌의 CRUD 화면)로 바로 넘겨버리는 빈 라우트였다. 사용자가 제공한 UI/UX 리뉴얼 스펙(상단 네비게이션 + Hero 검색 + 카테고리 + 추천 여행지 + 내 여행계획 지도위젯 + 빠른 시작 + AI CTA + 최근 본 여행)에 맞춰 `/`를 실제 대시보드로 새로 만들었다. 범위는 사용자와 사전 확인한 대로: **홈 화면에만** 새 상단 네비를 적용하고(다른 페이지는 기존 그대로), 카테고리/날씨/최근 본 여행은 목업이 아니라 **스키마 확장 + 실 API 연동**으로 구현했다.

- **DB 스키마 확장**(하위호환, 기존 기능 무변경): `Trip.tags String[]`(홈 카테고리 필터용, 생성 시 선택 안 해도 됨) + 신규 `TripView` 모델(사람×여행당 조회 시각 1건, upsert) 추가. `createTripSchema`/`sharedTripsQuerySchema`에 `tags`/`tag` 필드 추가(둘 다 optional이라 기존 API 호출은 그대로 동작)
- **카테고리(바다/맛집/카페/액티비티/자연/도시)**: [TripCreateForm.tsx](apps/web/src/app/trips/TripCreateForm.tsx)에 태그 선택 칩 추가(선택 안 해도 생성 가능, 기존 생성 플로우 안 건드림) → 홈 화면 `?category=` 쿼리로 `listPopularSharedTrips`가 `tags: { has: tag }`로 필터링
- **날씨**: 별도 API 키가 필요 없는 [Open-Meteo](apps/web/src/lib/weather.ts)로 "오늘의 추천 여행지"(코드에 고정된 7개 여행지 중 날짜로 순환 선택, [destinations.ts](apps/web/src/lib/destinations.ts))의 실시간 기온/날씨를 표시 — 크리덴셜 설정 없이 바로 동작
- **추천 여행지**: 기존 "다른 사람 여행계획" 탭(`listSharedTrips`, 최신순)은 그대로 두고, 좋아요 많은 순으로 정렬하는 `listPopularSharedTrips`를 새로 추가해 홈에서만 사용(기존 탭 정렬 안 건드림)
- **내 여행계획 지도 위젯**: `getFeaturedTripForHome`(가장 가까운 예정 여행, 없으면 최근 여행)의 장소 좌표를 기존 `KakaoMapCanvas`에 그대로 재사용(컴포넌트 수정 없이 컨테이너 높이만 지정)
- **최근 본 여행**: [tripViews.ts](apps/web/src/lib/services/tripViews.ts) 추가, 내 트립 상세(`/trips/[tripId]`)와 공개 트립 상세(`/trips/shared/[tripId]`) 두 곳에서 열람 시 `recordTripView` 호출
- **지도/AI여행계획/저장한 장소 네비**: 신규 페이지를 만드는 대신 기존 기능에 연결(사전에 사용자와 확정) — 지도→내 최근/예정 여행 상세, AI여행계획→그 여행의 기존 `/import` 파싱 화면, 둘 다 없으면 `/trips`로. 저장한 장소는 실제 기능이 없어 클릭 시 "준비 중인 기능이에요" 안내만 표시(가짜 페이지를 새로 만들지 않음)
- 새 홈 전용 컴포넌트는 `components/home/`에 모아 헤더/메시지·알림 아이콘 등을 새로 작성(기존 `NotificationNavLink`/`MessageNavLink`/`TripsTabs` 등은 스타일이 달라 그대로 재사용 안 하고 복제) — `/trips` 등 기존 화면 스타일에 영향 없음
- `SharedTripBrowser.tsx`는 홈 검색창(`/trips?tab=shared&q=...`)이 실제로 동작하도록 초기 검색어를 URL의 `q`에서 읽는 한 줄만 추가(그 외 기존 동작 무변경)

**발견한 문제와 원인**
- `npx prisma migrate dev`가 `20260907120000_add_trip_visibility_and_shares` 체크섬 불일치로 리셋을 요구함 — "다음 세션 할 일"에 이미 기록돼 있던, 스키마 변경 때마다 반복되는 기존 드리프트(이번이 5번째)였다. **DB를 리셋하지 않고** `prisma migrate diff`로 순수 추가분(SQL)만 뽑아 새 마이그레이션 폴더에 손으로 넣고 `migrate deploy`로만 적용해 우회(기존 세션들의 우회 방식과 동일). 근본 해결(드리프트 마이그레이션 체크섬 정정)은 여전히 미해결 — 아래 "다음 세션 할 일"에 유지
- Windows에서 `npx prisma generate`가 실행 중이던 dev 서버의 쿼리 엔진 DLL을 잠그고 있어 `EPERM` 에러 발생 → dev 서버를 먼저 내리고(`./down.sh`) generate 후 재기동하는 순서로 해결(스키마 변경 후엔 항상 서버 재시작 필요 — 기존에도 기록돼 있던 사항)

**검증 방법**
- `tsc --noEmit`/`eslint` 전체 통과(내가 안 건드린 기존 파일들의 에러/경고는 그대로 — 새로 만들거나 수정한 파일만 따로 lint해서 0 error 확인)
- 테스트 계정 1개로 브라우저 E2E: 신규 가입 직후 홈 화면에 실제 공개 여행 3건(다른 사용자들의 기존 데이터)이 좋아요순으로 뜨는 것, 오늘의 추천 여행지(속초) 날씨가 실시간 값(☀️ 23° 맑음)으로 뜨는 것, 여행이 없을 땐 "내 여행계획"/네비/빠른시작/AI CTA가 전부 `/trips`로 안전하게 폴백하는 것 확인 → `#바다` 태그로 여행 생성(대표사진 없음) + 전체공개 전환 → 홈에 그 여행이 태그와 함께 추천 목록에 뜨는 것, "내 여행계획" 위젯에 실제 카카오맵+오버레이 카드로 뜨는 것, "최근 본 여행"에 방금 연 여행이 뜨는 것, 지도/AI여행계획 네비가 그 여행 id로 바뀌는 것까지 확인 → 카테고리 "바다" 필터 클릭 시 해당 태그 여행만 남는 것, 다른 태그는 빈 상태 문구가 뜨는 것 확인 → 모바일 뷰(375px)에서 햄버거 메뉴가 열리고 6개 링크 전부 보이는 것 확인. 테스트 계정/여행은 정리 완료(cascade로 흔적 없음)

**후속 요청(같은 세션, 2026-09-16) — 로그인해도 새 홈 화면이 안 보임**

사용자가 실제로 로그인해보니 `localhost:3000`이 새 홈 화면이 아니라 예전 `/trips` 화면 그대로 뜬다고 알려줌. 서버 재시작이나 캐시 문제가 아니라, 로그인/회원가입/OAuth 성공 후 리다이렉트가 전부 `/`가 아니라 `/trips`로 하드코딩돼 있던 게 원인 — 새 홈(`/`)을 새로 만들면서 이 세 곳을 안 고쳤던 것.

- [login/page.tsx](apps/web/src/app/login/page.tsx): 로그인 성공 후 `router.push("/trips")` → `router.push("/")`
- [register/page.tsx](apps/web/src/app/register/page.tsx): 회원가입 성공 후 동일하게 `/`로 변경
...
**후속 요청(같은 세션, 2026-09-16) — `/trips`에서 홈으로 가는 버튼 추가**

`/trips`(내 여행계획 목록) 자체에는 홈으로 나가는 버튼이 없어서, 상단 우측 정렬돼있던 액션 줄(관리자/알림/메시지/로그아웃)을 양끝 정렬로 바꾸고 왼쪽에 "← 홈"(`/`) 링크를 추가했다. 기존 버튼들 스타일 그대로 재사용.
...
**완료 (2026-09-16, 로그인을 별도 페이지 대신 홈 우측 상단 팝업으로 전환)**

사용자가 로그인 화면을 홈 대시보드 우측 상단의 팝업으로 옮기고 싶다고 요청. 지금까지는 비로그인 시 홈(`/`) 자체가 없이 바로 `/login` 풀페이지로 리다이렉트했는데, "우측 상단 팝업"이 성립하려면 비로그인 상태에서도 보이는 홈 화면이 먼저 있어야 해서 두 가지를 함께 구현했다 — 사전에 모바일 처리/보호 라우트 목적지/회원가입 포함 여부 세 가지를 확인함(전체 옵션 다 추천안 선택).

- **게스트 홈**: `app/page.tsx`에서 비로그인 리다이렉트를 없애고, 로그인 여부로 완전히 다른 화면을 그린다 — 비로그인: Hero+카테고리+추천여행지(공개 데이터)만, "내 여행계획"/"빠른 시작"/"최근 본 여행" 같은 개인화 위젯은 숨김. 헤더도 [GuestHomeHeader.tsx](apps/web/src/components/home/GuestHomeHeader.tsx)로 교체해서 로그인해야만 쓸 수 있는 지도/둘러보기/AI여행계획 등 메뉴 자체를 안 보여줌(로고 + "로그인" 버튼만)
- **로그인 팝업**: [LoginPopupContext.tsx](apps/web/src/components/auth/LoginPopupContext.tsx)로 열림 상태를 공유해서, 헤더의 "로그인" 버튼뿐 아니라 Hero 검색창 제출·AI 배너 버튼처럼 서로 다른 위치의 트리거가 전부 같은 팝업 하나를 연다(검색/AI배너는 원래 `/trips`로 이동시켰는데 그 페이지 자체가 로그인 필요라 제출을 가로채 팝업을 여는 것으로 변경). 폼 로직은 `/login` 풀페이지와 공유하도록 [LoginForm.tsx](apps/web/src/components/auth/LoginForm.tsx)로 분리(중복 구현 안 함), 팝업엔 로그인만 넣고 "회원가입"은 기존 `/register` 풀페이지로 링크만
- **보호된 페이지 리다이렉트**: `/trips`, `/account`, `/messages`, `/notifications`, `/admin`, `/saved-places`, `/users/[nickname]` 등 14곳의 `redirect("/login")`을 전부 `redirect("/")`로 변경 — 비로그인으로 직접 들어와도 이제 로그인 화면이 아니라 게스트 홈으로 보내고 그 자리서 팝업으로 로그인하게 함
- **비로그인 좋아요 처리**: `listPopularSharedTrips`의 `viewerUserId`를 optional로 바꾸고, `DestinationCard`에 `viewerLoggedIn` prop 추가 — 게스트에게는 좋아요 버튼(로그인 기능) 대신 숫자만 보여줌
- **버그 발견/수정**: 모바일 전체화면 모달로 처음 구현했을 때, 팝업이 화면 전체가 아니라 헤더 높이(64px) 안에 갇혀 뜨는 버그 발견 — 헤더에 걸린 `backdrop-blur`가 `fixed` 자손의 CSS containing block을 만들어버린 것이 원인(`Modal.tsx` 주석에 이미 기록돼 있던 것과 동일한 문제 유형). `Modal.tsx`와 같은 방식으로 `createPortal`을 `document.body`에 써서 해결, 데스크톱 앵커는 절대위치 대신 `md:items-start md:justify-end` 정렬로 단순화
- `tsc`/`eslint` 전체 통과(신규/수정 파일 대상 lint 0 error, 프로젝트 전체 lint 결과가 기존 알려진 19건 경고/에러와 완전히 동일해 회귀 없음 확인). 테스트 계정으로 브라우저 E2E: 회원가입→로그아웃→게스트 홈(메뉴 없이 로그인 버튼만) 확인 → 데스크톱에서 팝업으로 재로그인해 페이지 이동 없이 바로 대시보드로 전환되는 것 확인 → 로그아웃 후 Hero 검색창 제출·AI 배너 버튼 클릭 시 각각 팝업이 뜨는 것 확인 → 모바일(375px)에서 로그인 버튼 클릭 시 전체화면 모달로 정상 표시되는 것(버그 수정 전/후 스크린샷으로 비교) → 비로그인 상태로 `/trips`/`/account`/`/saved-places` 직접 접근 시 `/`로 307 리다이렉트되는 것(curl로 확인)까지 확인. 테스트 계정 정리 완료

**완료 (2026-09-17, 여행 상세 화면을 "Travel Planning Workspace" 톤으로 UI 리뉴얼 — 기능 변경 없음)**

사용자가 여행 상세(`/trips/[tripId]`, `/trips/shared/[tripId]`) 화면 목업(히어로 배너+지도 72:28+날짜 탭+타임라인 카드+AI 카드+여행 요약)을 주고 "그대로 따를 필요 없이 우리 프로젝트에 맞게 판단해서" 계획을 세워달라고 요청. 기존 드래그앤드롭/날짜 아코디언 로직을 건드리면 안 되는 부분과 순수 리스타일로 되는 부분을 구분해서 진행했고, 목업에 없던 요구사항(지도 위 이동시간 라벨을 클릭하면 그 경로만 노란색으로 강조)도 추가로 반영했다.

- **날짜 선택**: 겉보기만 탭 모양 필(pill)로 바꾸고 실제 개폐 로직(`expandedDays` Set, 여러 날짜 동시 오픈 가능)은 완전히 그대로 유지 — 날짜 간 드래그 이동이 두 날짜가 동시에 펼쳐져 있어야 동작하기 때문(기능 변경 금지 원칙 확인 후 결정)
- **지도 강화**(`KakaoMapCanvas.tsx`, 기존 호출부엔 영향 없도록 전부 optional 필드로 추가): 번호+날짜색 원형 커스텀 마커, 경로 중간에 "🚗 시간·거리" 라벨 오버레이, 라벨 클릭 시 그 구간만 노란색(`#FACC15`)으로 강조·재클릭 시 해제 — 강조는 지도를 통째로 다시 그리지 않고 폴리라인 색만 바꾸는 별도 effect로 처리해 클릭할 때마다 확대/이동 상태가 리셋되지 않게 함. 마커 클릭 정보창에도 사진 썸네일·비용 표시 추가
- **레이아웃**: "전체화면 지도 위에 사이드바가 떠 있는" 기존 구조를 "히어로 배너 → 지도 72% : 타임라인 패널 28%"의 일반 스크롤 페이지로 변경. 히어로는 대표사진 배경 위에 기존 `TripMetaEditor`(수정/공유/공개범위, 로직 변경 없음)를 흰 카드로 얹고, 우측에 기존 "AI로 일정 가져오기" 링크를 리스타일한 [AIAssistantCard.tsx](apps/web/src/app/trips/[tripId]/AIAssistantCard.tsx) 배치
- **여행 요약 카드**: [TripSummaryCard.tsx](apps/web/src/app/trips/[tripId]/TripSummaryCard.tsx) 신규 — 이동 시간/거리는 지도 라벨과 같은 구간 API 결과를 합산(새 계산 로직 아님, 값 재사용), 방문 장소·예상비용은 기존 계산값 그대로
- **타임라인 카드**: 번호 배지를 날짜색으로, 사진 첫 장 썸네일, 카테고리 뱃지 추가하고 카드를 rounded-2xl+그림자로 감쌈. `RouteSegmentRow`/`ExpenseButton`/`PlacePhotosInline`은 기존 `ml-7` 들여쓰기를 새 카드 패딩에 맞게 제거한 것 외 로직 무변경
- **적용 범위**: 사용자 확인대로 `TripWorkspace.tsx`(편집 가능)와 `SharedTripView.tsx`(읽기전용) 둘 다 동일 컴포넌트(TripHeroBanner/AIAssistantCard/TripSummaryCard)를 재사용해서 통일 — "다른 사람 여행계획" 등 트립 목록 쪽 화면(TripGridCard 등)은 이번 범위 아니라 안 건드림
- **버그 발견/수정**: 모바일에서 지도가 안 보이는 버그 발견 — `flex-1`(flex-basis 0%)과 명시적 `h-[420px]`를 같은 요소에 같이 줬더니, 부모가 고정 높이가 없는 모바일(`flex-col`)에서는 `flex-1`이 이겨서 지도가 0높이로 접혀버림. `flex-1`을 `lg:flex-1`로(데스크톱 가로 배치일 때만) 옮겨서 해결
- `tsc`/`eslint` 전체 통과(신규/수정 파일 0 error, 프로젝트 전체 lint가 기존 19건 대비 지도 컴포넌트에 정당하게 늘어난 `any`/ref-cleanup 경고 2건만 추가 — 카테고리 자체는 기존과 동일). 테스트 계정으로 브라우저 E2E: 3일 여행 생성 → 서로 다른 날짜에 장소 추가 → 번호 마커·경로 라벨·비용 입력 후 정보창에 반영되는 것, 라벨 클릭 시 노란 강조가 지도를 리셋하지 않고 안정적으로 유지되는 것(재스크린샷 비교), 여행 요약 카드가 실제 구간 합계(189분·224.2km)와 지출(5,000원)로 정확히 갱신되는 것, 비용 탭 스타일, 모바일(375px)에서 지도가 정상 노출되는 것(버그 수정 전/후 비교)까지 확인 → 트립을 전체공개로 전환해 `/trips/shared/[id]`에서도 동일한 히어로/지도/요약이 뜨는 것 확인. **드래그앤드롭 자체는 이 자동화 브라우저의 합성 포인터 이벤트로 안정적으로 재현되지 않아(이 프로젝트에서 이미 알려진 자동화 한계) 육안 재현 검증은 못 했음 — 대신 `useSortable`/`DndContext`/`onDragEnd` 등 로직 코드는 한 줄도 안 건드리고 className만 바꿨는지 diff로 직접 확인**. 테스트 계정 정리 완료

**완료 (2026-09-17, 여행 상세 화면의 헤더·레이아웃 비율을 참고 이미지에 맞춤)**

사용자가 새 참고 이미지를 주며 (1) 지도/패널/헤더 비율을 이미지에 맞추고 (2) 상단 헤더를 홈 화면과 같은 걸 쓰자고 요청.

- **헤더 통일**: `/trips/[tripId]`·`/trips/shared/[tripId]` 페이지가 자체 "← 뒤로가기"만 있던 상단 대신, 홈/`/trips`와 똑같은 `HomeHeader`(로고+지도/둘러보기/내여행계획 등 네비+검색+알림+메시지+프로필)를 그대로 씀 — `active="trips"`, "지도"/"AI 여행계획" 네비는 지금 보고 있는 이 트립 자체를 가리키도록 연결. 헤더가 필요로 하는 알림/메시지 안읽음 수는 각 `page.tsx`(서버 컴포넌트)에서 조회해서 내려줌
- **히어로/AI카드 배치 변경**: AI 카드가 히어로 사진 위에 반투명으로 겹쳐 있던 것을, 참고 이미지처럼 히어로 옆에 완전히 분리된 카드로 이동(그라디언트 배경으로 리스타일). 히어로·AI카드 행과 지도·패널 행에 같은 폭(`flex-1` / `420px` 고정)을 똑같이 적용해서 위아래 두 행의 좌우 경계가 픽셀 단위로 맞도록 함
- `<main>` 태그가 페이지(`page.tsx`)와 클라이언트 컴포넌트 양쪽에 중복 렌더링되고 있던 것을 발견해 클라이언트 쪽을 `<div>`로 정정(시맨틱 HTML 오류였고 화면상 차이는 없었음)
- `tsc`/`eslint` 통과, 지도 컴포넌트 lint는 이전과 동일(회귀 없음). 테스트 계정으로 실제 헤더 네비/검색/알림 아이콘이 뜨는 것, 히어로와 AI 카드가 데스크톱에서 나란히(같은 높이) 뜨고 모바일에서 위아래로 쌓이는 것, `getBoundingClientRect()`로 히어로 폭(781px)+AI카드 폭(420px)이 아래 지도/패널 행과 동일한 위치에 정렬되는 것을 직접 확인, `/trips/shared/[id]`도 동일하게 적용된 것 확인. 테스트 계정 정리 완료

**완료 (2026-09-17, `/trips`의 "다른 사람 여행계획" 탭을 상단 네비 "둘러보기"로 통합)**

사용자 요청: 내 여행계획 페이지 탭 줄에 있던 "다른 사람 여행계획"을 상단 네비의 "둘러보기"로 옮기고 기존 탭은 삭제.

- 확인해보니 "둘러보기" 네비는 이미 `/trips?tab=shared`로 연결돼 있어서(홈 화면 작업 때 만들어둔 링크) 실제로는 같은 화면을 가리키고 있었음 — 새로 만들 것 없이 [TripsTabs.tsx](apps/web/src/app/trips/TripsTabs.tsx)에서 "다른 사람 여행계획" `TabButton`만 제거
- `tab === "shared"` 렌더링 분기(`SharedTripBrowser`)는 그대로 남겨둠 — "둘러보기"로 들어왔을 때(`?tab=shared`) 여전히 같은 내용을 보여줘야 하기 때문. 버튼만 없앤 것이라 URL로 들어오면 정상 동작
...
- **(중요)** 여행 상세 화면 리뉴얼(히어로/지도 강화/타임라인 카드) 이후, 장소 드래그 순서변경·날짜 간 드래그 이동이 실제 브라우저에서 여전히 정상 동작하는지 사람이 한 번 클릭·드래그로 확인 필요 — 자동화 브라우저의 합성 포인터 이벤트로는 재현이 안 돼 코드 diff(로직 미변경)로만 확인했음
- (신규) 여행 상세의 날짜 선택을 "겉보기만 탭 스타일 필"로 남겨둠 — 여러 날짜를 동시에 펼 수 있어서(교차 드래그 때문에 의도적으로 유지) 사용자에 따라 진짜 탭(한 번에 하나만 열림)을 기대할 수 있음. 필요해지면 "드래그 중일 때만 다른 날짜를 임시로 펼치는" 방식으로 진짜 탭처럼 바꾸는 것 검토
