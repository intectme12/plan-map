...
**완료 (2026-09-04, 지출 입력 폼 레이아웃 + 오른쪽 패널 토글)**
- `ExpenseButton`의 지출 추가 폼에서 금액 입력을 카테고리 select/추가 버튼과 한 줄에 억지로 욱여넣던 걸 2줄로 분리 — 윗줄은 카테고리+추가 버튼, 금액 입력은 그 아래 `w-full`로 폭 전체 사용
